...
/***************************插入数据***********************/
bool TemplateData::insertData(QString name, QString templateText)
{
    if(!db.isOpen())
    {
        db.open();
    ...
    QSqlQuery query;
    QString Str = QString("insert into PrintTemplate values('%1','%2');").arg(name, templateText);
...
/***************************查询数据***********************/
int TemplateData::selectData()
{
    int count = 0;
    if(!db.isOpen()) db.open();

    QSqlQuery query;
    QString str = QString("select * from PrintTemplate;");
...
/***************************更改数据***********************/
bool TemplateData::updateData(QString name, QString templateText)
{
    if(!db.isOpen())
    {
        db.open();
    ...
    QSqlQuery query;
    QString Str = QString("update  PrintTemplate set Template = '%1' where Name = '%2';").arg(templateText, name);
    bool success = query.exec(Str);  //执行sql语句
...
/***************************删除数据***********************/
bool TemplateData::deleteData(QString name)
{
    if(!db.isOpen())
    {
        db.open();
    ...
    QSqlQuery query;
    QString Str = QString("delete from PrintTemplate where Name = '%1';").arg(name);
    bool success = query.exec(Str);  //执行sql语句
